...
import 'package:fawry_app/app/data/model/invoice_purchase_History_Model.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';

// import 'package:lancera/app/data/models/user_model.dart';
  invoice_purchase_history_Model? invoicesModel;
Future<invoice_purchase_history_Model> InvoicesPurchaseByDate(
...
final Map<String, dynamic> Data = {
  ...
  final http.Response response = await http.post(
    Uri.parse('${APIUrl}ItemPurchasesListByDate'),
 ...
 var status = jsonDecode(response.body.toString())['code'].toString();
  ...
  if (status== '200' ) {
   ...
   print(jsonString);
invoicesModel = invoice_purchase_history_Model.fromJson(json.decode(response.body));

    return invoice_purchase_history_Model.fromJson(json.decode(response.body));
  } else {
